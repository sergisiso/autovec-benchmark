...
def read_data(folder_name):
    ...
    for filename in filter(lambda x: x.split('-')[0] == 'output',
...
def plot_per_platform(data):
    ...
    for platform in platforms:
        ...
        ax.legend((bars1[0], bars2[0], bars3[0], bars4[0]), (
            ...
            'Compile Time Exposed information\nauto-vectorization'),
            loc='upper left')
        fname = os.path.join(os.path.join('plots', 'perplatform'),
                             platform+'.eps')
...
def plot_per_test(data):
    ...
    for test, testdict in data.items():
        ...
        bars1 = ax.bar(xticks - 1.5 * width, sp_rt_novec, width,
                       color=colorm[0], edgecolor='black',
                       hatch='//', align='center')
        bars3 = ax.bar(xticks + 0.5 * width, sp_ct_novec, width,
                       color=colorm[2],  edgecolor='black', hatch='',
                       align='center')
        bars2 = ax.bar(xticks - 0.5 * width, sp_rt_vec, width,
                       color=colorm[1],  edgecolor='black', hatch='-',
                       align='center')
        bars4 = ax.bar(xticks + 1.5 * width, sp_ct_vec, width,
        ...
        #    shadow=True, ncol=2)
        fname = os.path.join(os.path.join('plots', 'perbenchmark'),
                             test+'.eps')
